...
3. Enterprise publishes open kind 28200 session event (no npub, just client_id)
...
1. Enterprise publishes addressed kind 28200 (tagged with specific agent_npub)
2. Agent notifies human: "Amazon wants to authorize me"
3. Human signs kind 28250 with their own NOSTR client
4. Human publishes kind 28250 to relay
5. Enterprise catches kind 28250, validates signature
...
1. Agent calls `POST /v1/membership/enroll/commit` with:
   ...
   - `authorization_event` (kind 28200 from enterprise)
...
2. **Agent publishes kind 28101** — Contains proof + public_outputs (merkle_root, npub) + delegation_id
3. **Enterprise catches kind 28101** — Subscribed to relay for proof events
4. **Enterprise validates delegation** — Queries NOSTR for kind 28250, checks expiry, checks for revocation
...
async function validateDelegation(agentNpubHex, delegationId) {
  ...
  // 3. Check for revocation (kind 28251)
...
- The agent signed the kind 28101 proof_event with the nsec derived from leaf_secret
...
If the human publishes kind 28251, the delegation is instantly revoked:

1. Human publishes kind 28251 tagged with delegation_id
...
| Revocation | Kind 28251 published | None |
